...
function generateHTML(data) {
    ...
    var parts = [
        ...
        '            col.innerHTML = `<div class="card h-100 p-4 shadow-sm section-tile ${isC ? \'bg-soft-danger\' : \'bg-white\'}" style="cursor:pointer;" onclick="showTab(\'stores\'); document.getElementById(\'storeSearch\').value=\'\'; renderStoreList();">',
        '                <div class="section-icon-wrapper"><div style="width:24px; height:24px;">${icon}</div></div>',
        '                <div class="section-score ${isC ? \'text-danger\' : \'text-primary-custom\'}">${sc.toFixed(1)}</div>',
        '                <div class="section-label mb-3" title="${k}">${k}</div>',
