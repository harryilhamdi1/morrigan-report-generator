...
function generateHTML(data) {
    ...
    var parts = [
        ...
        '    <div class="d-flex justify-content-between align-items-center mb-4">',
        '         <div><h2 class="text-primary-custom mb-0">Regional Analysis</h2><p class="text-muted m-0">Comparative insights to drive regional strategy</p></div>',
        '    </div>',
        '    <!-- Top Charts -->',
        '    <div class="row mb-4">',
        '        <div class="col-lg-6">',
        '             <div class="card h-100">',
        '                <div class="card-header-clean">🏆 Regional Leaderboard (Current Wave)</div>',
        '                <div class="card-body"><div id="regionLeaderboardChart" style="height: 350px;"></div></div>',
        '             </div>',
        '        </div>',
        '        <div class="col-lg-6">',
        '             <div class="card h-100">',
        '                <div class="card-header-clean">📈 Performance Trends</div>',
        '                <div class="card-body"><div id="regionTrendChart" style="height: 350px;"></div></div>',
        '             </div>',
        '        </div>',
        '    </div>',
        '    <!-- Weakness Matrix -->',
        '     <div class="card mb-5">',
        '        <div class="card-header-clean">🧩 Regional Weakness Heatmap (Sections)</div>',
        '        <div class="card-body"><div id="regionSectionHeatmap" style="height: 450px;"></div></div>',
        '     </div>',
        '    <!-- Cards -->',
        '    <h4 class="text-primary-custom mb-3">Regional Deep Dive</h4>',
        ...
        '   var lbData = regKeys.map(r => {',
        '       var d = reportData.regions[r][curWave];',
        '       return { r: r, s: d ? d.sum/d.count : 0 };',
        '   }).sort((a,b) => a.s - b.s);',
        '   var colors = lbData.map(d => d.s >= 84 ? "#4A7FB5" : "#EF4444");',
        '   Plotly.newPlot("regionLeaderboardChart", [{',
        '       y: lbData.map(d => d.r), x: lbData.map(d => d.s),',
        '       type: "bar", orientation: "h", marker: {color: colors},',
        '       text: lbData.map(d => d.s.toFixed(2)), textposition: "auto"',
        '   }], { margin: {l: 150}, xaxis: {range: [60, 105]} }, config);',
        '',
        '   // 2. Trends',
        '   var traces = regKeys.map(r => {',
        '       var y = waves.map(w => { var d = reportData.regions[r][w]; return d ? d.sum/d.count : null; });',
        '       return { x: waves, y: y, mode: "lines+markers", name: r, visible: "legendonly" };',
        '   });',
        '   // Make top 3 visible by default',
        '   traces.slice(0, 3).forEach(t => t.visible = true);',
        '   Plotly.newPlot("regionTrendChart", traces, { showlegend: true, legend: {orientation: "h", y: -0.2} }, config);',
        '',
        '   // 3. Section Heatmap',
        ...
        '           if(!d) return null;',
        '           var sd = d.sections[s];',
        '           return sd ? sd.sum/sd.count : null;',
        '       });',
        '   });',
        '   Plotly.newPlot("regionSectionHeatmap", [{',
        '       x: secKeys.map(s => s.split(" ")[0]), y: regKeys, z: z,',
        '       type: "heatmap", colorscale: [[0, "#EF4444"], [0.83, "#FCD34D"], [0.84, "#10B981"], [1, "#047857"]],',
        '       hovertemplate: "Region: %{y}<br>Section: %{x}<br>Score: %{z:.2f}<extra></extra>"',
        '   }], { margin: {l: 120, b: 50} }, config);',
        '',
        '   // 4. Detail Cards',
        ...
        '                   <h5 class="fw-bold text-primary-custom mb-3">${r}</h5>',
        '                   <div class="d-flex align-items-end justify-content-between mb-4">',
        '                       <div>',
        '                           <div class="display-5 fw-bold ${score < 84 ? "text-danger" : "text-dark"}">${score.toFixed(2)}</div>',
        '                           <div class="small ${diff>=0 ? "text-success" : "text-danger"} fw-bold">',
        '                               ${diff>=0 ? "▲" : "▼"} ${Math.abs(diff).toFixed(2)} vs last wave',
        '                           </div>',
        '                       </div>',
        '                       <div>',
        '                           <div class="text-end text-muted small text-uppercase fw-bold">Active Stores</div>',
        '                           <div class="h4 mb-0 text-end">${d.count}</div>',
        '                       </div>',
        '                   </div>',
        '                   <div class="p-3 bg-light rounded-3 mb-3" style="border-left: 4px solid #EF4444;">',
        '                       <div class="text-muted small text-uppercase fw-bold mb-1">Focus Area (Lowest Section)</div>',
        '                       <div class="fw-bold text-danger text-truncate" title="${minSec}">${minSec}</div>',
        '                       <div class="h5 mb-0 mt-1">${minVal.toFixed(2)}</div>',
        '                   </div>',
        '                   <button class="btn btn-outline-primary w-100 btn-sm" onclick="showTab(\'stores\'); document.getElementById(\'storeSearch\').value=\'${r}\'; renderStoreList();">View Stores</button>',
        '               </div>',
        '           </div>`;',
